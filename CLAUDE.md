...
- Task Runner: `package.json` + `pnpm` (管理 lint / test / clean / ci 與領域任務進入點)
...
| 硬體與系統狀態偵測 (Hardware & System Probe)      | `scripts/system/`                                                                                                         | `pnpm run run:system:show`, `pnpm run run:system:<information>`, `pnpm run run:system:disk-verify`；純腳本：`scripts/system/*.sh` |
| 開發環境清單同步 (Development Manifest Sync)      | `scripts/dump/`, `scripts/Brewfile`, `bin/vscode/*_extension_list.txt`                                                     | `pnpm run run:dump:mac`, `pnpm run run:dump:vscode`, `pnpm run run:dump:antigravity`；純腳本：`scripts/dump/*.sh` |
| IDE 擴充套件同步 (IDE Extension Sync)             | `scripts/install/`, `bin/vscode/*_extension_list.txt`                                                                     | `pnpm run run:install:vscode`, `pnpm run run:install:antigravity`；純腳本：`scripts/install/*.sh` |
| macOS 設定備份 (macOS Defaults Backup)            | `scripts/backup/`                                                                                                         | `pnpm run run:backup`, `pnpm run run:backup:list`, `pnpm run run:backup:import`, `pnpm run run:backup:init`；純腳本：`scripts/backup/*.sh` |
| macOS Codex 移除 (macOS Codex Uninstall)          | `scripts/uninstall/`                                                                                                      | `pnpm run run:uninstall:codex`；純腳本：`scripts/uninstall/codex.sh`               |
| macOS 系統稽核與清理 (macOS Audit & Cleanup)      | `scripts/cleanup/`, `bin/mac/*_audit-mac.sh`                                                                              | `pnpm run run:cleanup`, `pnpm run run:cleanup:<target>`；純腳本：`scripts/cleanup/*.sh` |
...
本 repo 為純 Shell 腳本工具箱，無需二進位編譯步驟。工具直接編寫於 `scripts/` 或 `bin/<area>/<tool>` 並賦予執行權限 (`chmod +x`)；專案與領域任務由 `package.json` 的 pnpm scripts 統一管理。

### 測試 (Test)

- `./run.sh` 驗證 symlink 全部建立
- `pnpm run lint` 驗證 `scripts/` 與 `bin/` 所有 `.sh` 腳本之語法正確性 (`bash -n`)
- `pnpm test` 執行 `scripts/test_bash_plugin.sh` 驗證區塊替換 idempotent 單元測試
- `pnpm run test:docker` 於隔離的 Ubuntu 容器中驗證完整 toolchain 安裝與命令驗證（需本機 Docker 或 Colima 運作中）
...
本地與整合驗證透過 `pnpm run ci` (`pnpm run lint && pnpm run test`) 執行，確保所有 Shell 腳本無語法錯誤且 plugin 區塊管理具備冪等性；已移除 GitHub Actions 工作流程，所有驗證可在本機或容器內自主完成。
...
- 工具加入流程 (Scalability)：
    ...
    6. 若需 pnpm 統一入口，在 `package.json` 的 `scripts` 註冊 `run:<domain>:<action>`
