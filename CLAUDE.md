...
│   │   ├── mac_keyboard_shortcuts_dump / mac_keyboard_shortcuts_restore
│   │   ├── mac_extension_list
...
│   │   ├── backup / backupSync
│   │   ├── scan_private_network / scan_target_network / scan_devices
│   │   ├── ssoLogin.sh / ssoLogin_faas.sh
│   │   ├── claudew / goswitch
│   │   ├── bytedance_setup.sh     # ⚠️ 含明文密碼, 預計刪除
│   │   ├── ssh_config / sshd_config / ssh_keygen / ssh_key_compare
│   │   ├── ssh.md / scan_private_network.md / scan_private_network.output.sample
...
> 註：`cmd/` 目錄 (舊 README/CLAUDE.md 提及之 Go 1.25.4 + Cobra + logrus `smain`) 已不存在，相關敘述於本次重寫一併移除。
...
- **macOS 稽核與硬體偵測分流**：`bin/mac/` 為 macOS 專屬 (`mac_cleanup`、`*_audit-mac.sh`)；`bin/system/` 為跨平台硬體偵測 (`*_info`、`system_info`)。`bin/system/README.md` 目前標題錯置需修正。
- **不引入 Go/Cobra 框架**：早期 `cmd/smain` 入口已移除，repo 為純 shell + python 工具集；不混用 binary CLI。
...
| macOS 系統稽核與清理 (macOS Audit & Cleanup)       | `bin/mac/`                                                            | `./bin/mac/mac_cleanup`, `./bin/mac/disk_analysis-mac.sh`                                             |
...
- `git grep -n 'cmd/\|smain\|project_setup'` 確認無殘留敘述
