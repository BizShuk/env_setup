...
- `掃描本機所連私有網路拓樸 (Scan private network topology)` — 開發者執行 `bin/network/scan_network.sh --mode=private` 或 `--mode=topology`，traceroute 走到第一個公網 hop，再對最後私有 hop 跑 nmap。
...
- `網路掃描前置依賴 (Network scan deps)`：`bin/network/scan_network.sh` 與 `bin/system/network_topology_scan.sh` 啟動時 `command -v` 檢查 `traceroute` 與 `nmap`；缺一即 `exit 1` 拒絕執行。
